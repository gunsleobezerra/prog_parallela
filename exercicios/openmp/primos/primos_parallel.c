...
int main(int argc, char *argv[])
{
  ...
  nthreads= getenv("OMP_NUM_THREADS") ? atoi(getenv("OMP_NUM_THREADS")) : 1;
  ...
  #pragma omp parallel for reduction(+:nPrimes) // nesse caso consigo usar o parallel for pois para saber se um número é primo ou não não depende do resultado de outro número
